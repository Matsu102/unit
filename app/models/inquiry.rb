class Inquiry < ApplicationRecord
  ...
  validates :user_id,        numericality: { message: 'は数字で入力してください。' }, :allow_blank => true, if: -> { user_id.present? }
  validates :email,          presence: { message: 'を入力してください。' }
  validates :inquiry_name,   presence: { message: 'を30文字以内で入力してください。' }
  validates :inquiry_name,   length: { in: 1..30, message: 'に入力できるのは30文字までです。' }, if: -> { inquiry_name.present? }
  validates :inquiry_detail, presence: { message: 'を500文字以内で入力してください。' }
